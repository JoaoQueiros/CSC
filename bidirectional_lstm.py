...
# -*- coding: utf-8 -*-
"""
Created on Sun May 23 13:43:31 2021
...
def model_lstm(X_train, middle_layers, neurons, f_activation, amnesia, optm, loss_funct):
    ...
    regressor.add(Bidirectional(LSTM(units = neurons, activation=f_activation, return_sequences = True, input_shape = (X_train.shape[1], 1))))
    regressor.add(Dropout(amnesia))
    i = 0
    
    #Number of middle_layers
    while i < middle_layers:
        regressor.add(Bidirectional(LSTM(units = neurons, activation=f_activation, return_sequences = True)))
        regressor.add(Dropout(amnesia))
        i += 1
    
    regressor.add(Bidirectional(LSTM(units = neurons, activation=f_activation, return_sequences = False)))
    regressor.add(Dropout(amnesia))
...
"""
*********************************************************************
6)  Bidirectional LSTM MODEL FUNCTION TRAINING
...
save_file = "bi_lstm_model/obs_"+str(obs)+"_ml_"+str(middle_layers)+"_n_"+str(neurons)+"_f_"+str(f_activation)+"_am_"+str(amnesia)+"_opt_"+str(optm)+"_ep_"+str(epochs)+"_bs_"+str(batch_size)
