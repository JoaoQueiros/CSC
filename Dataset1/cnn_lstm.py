...
def model_lstm(X_train, middle_layers, neurons, f_activation, amnesia, optm, loss_funct):
    ...
    n_features = 1
    n_seq = 2
    n_steps = int(X_train.shape[1]/2)
    X_train = X_train.reshape((X_train.shape[0], n_seq, n_steps, n_features))
    # Initialising the RNN
    regressor = Sequential()
    # Adding the first LSTM layer and some dropout regularization
    regressor.add(TimeDistributed(Conv1D(filters=64, kernel_size=1, activation='relu'), input_shape=(None, n_steps, n_features)))
...
"""
*********************************************************************
6)  CNN-LSTM MODEL FUNCTION TRAINING
...
save_file = "cnn_lstm_model/obs_"+str(obs)+"_ml_"+str(middle_layers)+"_n_"+str(neurons)+"_f_"+str(f_activation)+"_am_"+str(amnesia)+"_opt_"+str(optm)+"_ep_"+str(epochs)+"_bs_"+str(batch_size)
